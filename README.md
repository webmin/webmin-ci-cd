...
Push code review remains part of the master build workflow so submitted code is checked before packages are built. Merged pull request changes are reviewed by the push workflow when they land on the target branch. When no code review API key is passed, the review job exits before checkout or script download. Commit comments are enabled by default for push reviews, while email notifications are available only when a child workflow explicitly sets `code-review-email: true`. Each emailed finding links to its commentable GitHub commit discussion; the inline GitHub Actions check annotation remains a read-only CI view.
...
- **gpg-preset-repositories.bash** — this script presets the GPG passphrases (both server-side and user/CI-side) into the repositories user's gpg-agent for repository signing. It ensures that the necessary passphrases are available in memory for signing operations, facilitating secure and automated package signing during the build process.
...
- **Installation**:
  ```bash
  curl -fsSL https://download.virtualmin.dev/repository | sh -s -- -s -B unstable
  ```
...
- **Description**:
  ...
  - Built automatically when a tagged release is created
  - Intended for **users** who want to try upcoming features before the final release
...
### **Virtualmin stable repository**
- **URL**: [download.virtualmin.com](https://download.virtualmin.com)
- **Description**:
  - Stable builds of Virtualmin, its plugins, and other related packages
  - Built automatically when a tagged release is created and promoted
  - Intended for **all users** who want the latest stable release

- **Installation**:
  ```bash
  curl -fsSL https://download.virtualmin.com/repository | sh -s -- -s -B stable
  ```
